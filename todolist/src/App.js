...
function App() {
  ...
  const onRemove = (index) => {
    ...
    setToDos((currentArray) => currentArray.filter((_, i) => {
      return i !== index
    }));
  };
  ...
  return (
    <>
      ...
      <form className='form-box' onSubmit={onSubmit}>
        <input id="todo" className='todo-input' type="text" value={editIndex !== null ? editText : toDo} onChange={(e) => (editIndex !== null ? setEditText(e.target.value) : setToDo(e.target.value))} />
        <span id="addBtn" className='add-btn' role='button' onClick={onSubmit} onKeyDown={(event) => { if (event.key === 'Enter') onSubmit(event); }}></span>
      </form>
      <div id='todoBox' className='todo-box'>
        {toDos.map((item, index) => (
          <label className={`new-todo ${item.checked ? "check" : ""}`} key={index}>
            {item.text}
            <input type='checkbox' checked={item.checked} onChange={() => onToggleCheck(index)} />
